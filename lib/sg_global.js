...
sg_global.prototype.create = function(fileName){
	...
	var m_html = helpers.renderFile(path.join(config.rootPath,"templates/m.ejs"),data);//把ejs模板传入数据、编译成html返回
	var pc_html = helpers.renderFile(path.join(config.rootPath,"templates/pc.ejs"),data);
